...
copied from the generated files, none are typed from memory.
...
- **Numbers (permutation metric, provisional until the deterministic report is regenerated):** Jena SHAP-ranked fidelity — Crossformer 0.420 ± 0.194, ours 0.401 ± 0.189, **our built-in attention 0.040 ± 0.230**. Beijing — iTransformer 0.273, Crossformer 0.269, ours 0.235, our attention 0.112.
- **What the paper must say:** the built-in attention is *not* a competitive explanation; the claim has to be dropped and the honest contribution restated — a model that is first on MAE whose attention is an internal mechanism, with post-hoc SHAP as the explanation of record.
...
- **Numbers:** the estimator's own noise on a single fixed checkpoint is sd ≈ 0.17–0.18 — as large as the spread across trained seeds, so at this budget the metric cannot separate models closer than ≈ 0.2. The published sd of 0.037 was an artefact of reusing one RNG stream. Hence the deterministic metric.
...
### Major #7 — the entropy penalty may manufacture the agreement it "validates" — **partly**
...
- **Numbers (faithfulness):** after Holm over the 12 tests nothing is significant and the direction is against the regulariser. What does move faithfulness is RevIN, not the penalty: attention-ranked fidelity 0.335 → 0.040 when RevIN is switched off (p = 0.004, 5/5 seeds).
- **Still missing:** the deterministic fidelity report has not been regenerated since `fidelity_det.csv` finished; that file belongs to the analysis session.
...
| 6 | Only one Beijing station; 12 available | **partly** — zero-shot transfer to the other 11 stations is running (`analysis/cross_station.py`), 204 of 660 runs done; the in-domain control reproduces `results_clean.csv` exactly |
| 7 | Missing-data robustness sweep (5/10/20 %) absent | **done** — `analysis/missing_robustness.md`: first on both datasets at 20 % scattered missingness; `analysis/block_missing.md` adds the block-outage case, where it is not |
| 8 | Hardware/versions not recorded | **done** — `analysis/reproducibility.md`, `analysis/environment.json`, `analysis/requirements_frozen.txt`: Tesla T4, Python 3.12.13, torch 2.10.0+cu128, CUDA 12.8, TSLib pinned at `4e938a17` |
...
2. **Deterministic fidelity report** (R1 Major #1, #4, #7): `fidelity_det.csv` is complete, but `xai_fidelity_v2.md` still prints "not found" and has to be regenerated — the analysis session owns that file.
3. **`analysis/tuning_budget.md`** still says 138 runs and calls Crossformer under-provisioned; both are superseded (300 runs, and width does not help it).
4. **Manuscript edits**: R1 Major #2, R1 Minor #1, #2, #3, #4, #5, #9, and the whole R2 reporting paragraph. None need computation; all need the text rewritten against the numbers above.
5. **If a recipe variant is ever promoted to the headline**, fidelity must be recomputed for it: 10 deterministic + 10 permutation runs, about 1 h locally.
...
2. *"Built-in attention is not a useful explanation."* Confirmed — his reading was
   right, and the evidence is now ours: the attention is flat on the time axis,
   unstable on the variable axis, and its fidelity is near zero. The paper's XAI
